---
title: 'What is the Delegate Pattern?'
date: '2023-08-02'
lastmod: '2023-08-02'
tags: ['design patterns', 'delegate', 'behavioral patterns']
draft: false
summary: 'A brief look at how to write more modular code with the delegate pattern.'
layout: PostLayout
---

## Overview

<TOCInline toc={props.toc} exclude="Overview" toHeading={(2, 3)}></TOCInline>

## What is the Delegate Pattern?

Design patterns play a crucial role in software development, enabling us to build scalable, maintainable, and flexible applications. They are reusable solutions to common problems that we encounter during the development process.

## How Does it Work?

At its core, the delegate pattern allows one object (the delegate) to communicate and customize the behavior of another object (the delegating object, or the delegator) without the need for the delegator to know the specific type of the delegate.

The delegate pattern is made up of **two** essential components:

1. **Delegate Protocol**

The delegate protocol (or the **delegator**) defines a set of methods that the delegating object should implement. Essentially, the protocol works like a contract. Any object acting as the delegate must conform to the methods defined in the protocol.

2. **Delegating Class/Object**

The delegating object (or, the **delegate**) contains specific implementation details related to the methods defined in the protocol (delegator). The methods in the delegate are only called whenever necessary by the delgator. This is known as a weak reference and it allows the delegator to remain unaware of the delegate's specific implementation details.

In other words, the delegator doesn't need to know the specific type of the delegate or how the delegate implements the methods defined in the protocol. This is what makes the delegate pattern modular.

Okay, but that was a lot of jargon. Let's look at an example to help us understand the delegate pattern a little better.

## Example Usage

Let's say that we're building a mobile app that allows users to customize photos. In this scenario, we could implement the delegate pattern to allow users to add filters to their photos.

### Define the Delegator

To do this, we can create a photo processing delegate protocol that defines a method called `applyFilter`. This method takes in a `UIImage` and returns a `UIImage` with the filter applied.

```swift
protocol PhotoProcessingDelegate {
    func applyFilter(to image: UIImage) -> UIImage
}
```

Notice that the delegate protocol doesn't specify how the filter should be applied. This allows us to customize the behavior of the filter within our delegate class.

### Create a Delegate Class

Here, the `Filter` class becomes the delegate because it conforms to the `PhotoProcessingDelegate` protocol. Which means that it must implement the `applyFilter` method as defined in the protocol earlier.

```swift
class Filter: PhotoProcessingDelegate {
    func applyFilter(to photo: UIImage) -> UIImage {
        // Apply filter to image
        return photo
    }
}
```

### Create a Class that Holds the Delegate

Next, we can create a `PhotoProcessor` class that holds a weak reference to the delegate and calls its `applyFilter` method whenever necessary.

```swift
class PhotoProcessor {
    weak var delegate: PhotoProcessingDelegate?

    func process(image: UIImage) {
        guard let delegate = delegate else { return }

        let filteredImage = delegate.applyFilter(to: image)
        // Do something with the filtered image
    }
}
```

The `PhotoProcessor` class doesn't need to know the specific type of the delegate. It only needs to know that the delegate conforms to the `PhotoProcessingDelegate` protocol.

### Establish the Connection

Now, we can create an instance of the `PhotoProcessor` class and set the `Filter` instance as its delegate.

```swift
let photoProcessor = PhotoProcessor()
photoProcessor.delegate = Filter()
```

### Call the Delegate Method

When we call the `process` method on the `PhotoProcessor` instance, it will call the `applyFilter` method on the `Filter` instance, passing in the image as an argument.

```swift
photoProcessor.process(originalImage)
```

## Advantages

As you probably noticed in the example, the delegate pattern promotes loose coupling. Or, in simpler terms, it allows the components to be connected in a flexible manner. The real power of the delegate pattern lies in its ability to allow us to customize the behavior of an object without the need to subclass it.

### Use Cases

The delegate pattern can become useful in a variety of scenarios. However, one of the most common use cases that comes to mind is dealing with UI components. Implementing the delegate pattern to handle the behavior of the UI components without tightly coupling it to the data model can be extremely useful. Additionally, you may find that using the delegate pattern can be helpful in a few other situations, including:

- Customizing the behavior of a class without subclassing
- One-to-one relationships
- Event handling
- Customizing the behavior of a view controller

## Limitations

While this pattern can be extremely useful, it's important to recognize when it's not worthwhile to implement it. For example, if you find yourself creating a delegate protocol with only one method, it may be better to use a closure instead. Additionally, if you find that the data is being tightly coupled to the delegate, it may be better to use a subclass. A few other scenarios that you should be wary of when using the delegate pattern are:

- Performance intensive operations
- One-to-many relationships
- Code complexity
- Retain cycles (when the delegate and delegator hold strong references to one another)

## Closing Thoughts

I hope this is a helpful introductory to the delegate pattern. If you have any questions or feedback, feel free to reach out to me on [Threads](https://threads.net/lyndipc_). Otherwise, research and experiment with the delegate pattern to see how you can use it in your own projects. Thanks for reading.

## Further Reading

- [Using Delegates to Customize Object Behavior - Apple Developer Docs](https://developer.apple.com/documentation/swift/using-delegates-to-customize-object-behavior)
- [Delegation Pattern in Swift - Anand Nimje](https://medium.com/@nimjea/delegation-pattern-in-swift-4-2-f6aca61f4bf5)
- [Delegate Pattern Discussion - r/swift on Reddit](https://www.reddit.com/r/swift/comments/yy5x15/delegate_pattern/)
- [Delegate Retain Cycle in Swift - Kaan Vural](https://medium.com/macoclock/delegate-retain-cycle-in-swift-4d9c813d0544)
